...
from ftmgram.types import InlineKeyboardMarkup, InputMediaPhoto, Message, InlineKeyboardButton
from ftmgram.errors import MessageIdInvalid, MessageNotModified
...
async def stream(client, _, mystic, user_id, result, chat_id, user_name, original_chat_id, video: Union[bool, str] = None, streamtype: Union[bool, str] = None, spotify: Union[bool, str] = None, forceplay: Union[bool, str] = None, userbot=None):
    ...
    if streamtype == "playlist":
        ...
        for search in result:
            ...
            if await is_active_chat(chat_id):
                ...
                button = panel_markup_clone(
                    _, vidid, chat_id, bot_username=bot_username
                )
                ...
                await attach_player_rich_message(
                    client, run, title=title, duration=duration_min,
        ...
        if await is_active_chat(chat_id):
            ...
            await attach_player_rich_message(
                client, run, title="Index or M3u8 Link", duration="Live",
                requested_by=user_name,
            )
            db[chat_id][0]["mystic"] = run
            db[chat_id][0]["markup"] = "tg"
